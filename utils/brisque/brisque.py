...
def asymmetric_generalized_gaussian_fit(x):
    ...
    def mean_squares_sum(x, filter = lambda z: z == z):
        ...
        if filtered_values.shape[0] == 0.0:
            return 0.0
        return squares_sum / filtered_values.shape[0]
...
def scale_features(features):
    with open('utils/brisque/normalize.pickle', 'rb') as handle:
...
def getBRISQUE(imgs):
    ...
    if 1:
        ...
        for j in range(temp.shape[2]):
            brisque_features = calculate_brisque_features(temp[:,:,j], kernel_size=7, sigma=7/6)
            downscaled_image = cv2.resize(temp[:,:,j], None, fx=1/2, fy=1/2, interpolation = cv2.INTER_CUBIC)
            downscale_brisque_features = calculate_brisque_features(downscaled_image, kernel_size=7, sigma=7/6)
            brisque_features = np.concatenate((brisque_features, downscale_brisque_features))
            BRISQUE = calculate_image_quality_score(brisque_features)
            bsq += BRISQUE
    bsq = bsq/(imgs.shape[0]*3)
    return bsq
